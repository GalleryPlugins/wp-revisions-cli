...
class Revisions_CLI extends WP_CLI_Command { // phpcs:ignore WordPressVIPMinimum.Classes.RestrictedExtendClasses.wp_cli
    ...
    public function clean( $args = array(), $assoc_args = array() ) {
        ...
        // Check if it is a multisite installation
        if ( is_multisite() ) {
            echo 'is_multisite:true';
            // Get all blog ids
            ...
            foreach ( $blog_ids as $blog_id ) {
                ...
                // Call the clean-up operation for the current blog
                $this->_clean_single_site( $args, $assoc_args );

                // Restore the current blog
                restore_current_blog();
            }
        } else {
            // If it's not a multisite, just perform the clean-up
            $this->_clean_single_site( $args, $assoc_args );
        }

        // Show success message
        WP_CLI::success( 'Finished removing old revisions for all sites.' );
    }
    public function _clean_single_site( $args = array(), $assoc_args = array() ) {
